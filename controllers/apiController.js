...
var api ={};
// 服务列表
api.serverList = function(req,res){
    var list = [];
    for(var i = 0;i<config.Config.length;i++){
        list.push({
            QLkey:config.Config[i].QLkey,
            QLName:config.Config[i].QLName,
            QL_CAPACITY:config.Config[i].QL_CAPACITY,
            QRurl:config.Config[i].QRurl
        })
        
    }
    // console.log(`${config.Config[0].QLurl}/open/auth/token?client_id=${config.Config[0].QL_CLIENTID}&client_secret=${config.Config[0].QL_SECRET}`)
    // console.log(config.Config[3])
    var e=request({url:`${config.Config[0].QLurl}/open/auth/token?client_id=${config.Config[0].QL_CLIENTID}&client_secret=${config.Config[0].QL_SECRET}`,
    method:'GET',
    headers:{'Content-Type':'text/json' }
    },function(error,response,body){
        var bodydata = JSON.parse(body)
        console.log(bodydata)
        if(bodydata.code==400){
            res.send({code:400,message:bodydata.message,list:list,title:config.Title,Announcement:config.Announcement});
            res.end();
        }else if(!error && response.statusCode==200){
            console.log(JSON.parse(body))
            res.send({code:200,token:bodydata.data.token,list:list,title:config.Title,Announcement:config.Announcement});
            res.end();
        }
    });
...
api.serverDetails = function(req,res){
    var number = req.query.number?req.query.number:3;
    var token = req.query.token?req.query.token:'';
    var e=request({url:`${config.Config[number].QLurl}/open/auth/token?client_id=${config.Config[number].QL_CLIENTID}&client_secret=${config.Config[number].QL_SECRET}`,
    method:'GET',
    headers:{'Content-Type':'text/json' }
    },function(error,response,body){
        if(!error && response.statusCode==200){
            console.log(JSON.parse(body))
            var bodydata = JSON.parse(body)
            var newtoken = bodydata.data.token
            var e=request(
                {url:`${config.Config[number].QLurl}/open/envs`,
                method:'GET',
                headers:{
                ...
                if(!error && response.statusCode==200){
                    ...
                    for(var i = 0;i<bodydata.data.length;i++){
                        if(bodydata.data[i].name=='JD_COOKIE'){
                            cookie.push(bodydata.data[i]);
                        }
                    }
                    var num = config.Config[number].QL_CAPACITY-cookie.length;
                    res.send({code:200,count:num,token:newtoken})
                    res.end();
                }
            });
        }
...
// 提交变量
api.serveritem = function(req,res){
    var name = "JD_COOKIE";
    ...
    var requestData=JSON.stringify([{
        name:name,
        remarks:'SaoBing:'+remarks,
        value:value
    }]);
    var uname =  decodeURIComponent(value.match(/pt_pin=(.+?);/) && value.match(/pt_pin=(.+?);/)[1])
    var e=request(
        {url:`${config.Config[number].QLurl}/open/envs?t=${Date.now()}`,
        method: 'POST',
        headers:{
        ...
        if(!error && response.statusCode==200){
            ...
            if(bodydata.code==200){
                // console.log(bodydata)
                res.send({code:200,data:bodydata,name:uname})
                res.end() 
            }else{
                res.send({code:400})
            }
...
var getCaption = function(obj,state){
...
// 提交变量
api.serveritemUp = function(req,res){
    var name = "JD_COOKIE";
    var remarks = req.body.remarks;
    var token = req.body.token;
    var value = req.body.value;
    ...
    if(getCaption(remarks,0)=="SaoBing"){
        var remarks = getCaption(remarks,1);
    }
    var requestData=JSON.stringify({
        name:name,
        remarks:'SaoBing:'+remarks,
...
api.serveritemDelete = function(req,res){
    var _id = req.body._id;
    var token = req.body.token;
    var number = req.body.number;
    var requestData=JSON.stringify([_id]);
    var e=request(
        {url:`${config.Config[number].QLurl}/open/envs?t=${Date.now()}`,
        method: 'DELETE',
        headers:{
...
api.zc = function(req,res){
    var cookiepost = req.body.cookie;
    if(cookiepost==null){
        res.send({code:400});
        res.end();
        return;
...
async function showMsg() {
    ...
    // console.log(`saobing${ReturnMessage}`)
    res.send({code:200,data:ReturnMessage});
    res.end();
	if ($.isNode() && WP_APP_TOKEN_ONE) {
		if (TempBaipiao) {
...
// 查询任务
api.task = function(req,res){
    var number = req.query.number;
    var token = req.query.token;
    console.log(number)
    console.log(token)
    var e=request(
        {url:`${config.Config[number].QLurl}/open/crons?searchValue=&t=${new Date()}`,
        method:'GET',
        headers:{
            'Content-Type':'application/json',
            'Authorization':`Bearer ${token}`,
        ...
        if(!error && response.statusCode==200){
            ...
            if(bodydata.code == 200){
                for(var i = 0;i<bodydata.data.length;i++){
                    data.push({
                        name:bodydata.data[i].name,
                        _id:bodydata.data[i]._id
                    })
                }
                res.send({code:200,data:data})
                res.end();
            }else{
                res.send({code:400,data:data})
                res.end();
            }
        }
    });
...
api.taskItem = function(req,res){
    ...
    console.log(name)
    var e=request(
        {url:`${config.Config[number].QLurl}/open/crons/${_id}/log?t=${new Date()}`,
        method:'GET',
        headers:{
            'Content-Type':'application/json',
            'Authorization':`Bearer ${token}`,
        ...
        if(!error && response.statusCode==200){
            ...
            var bodydata = JSON.parse(body)
            if(bodydata.code == 200){
                var str = subStringOne(bodydata.data,name,"京东账号")
                res.send({code:200,data:str})
                res.end();
            }else{
                res.send({code:400,data:"服务器决绝你的请求"})
                res.end();
            }
        }
    });
